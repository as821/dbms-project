...
def write(table_obj, obj_to_insert):
    # check existence of Storage object
    if type(table_obj.storage) is object:
        error("storage object not created.")
...
def create_index(table_obj, index_name, attr):
    if type(table_obj.storage) is object:
        error("storage object not created.")
    ...
    for r in range(len(relation)):
        ...
        if relation[r][attr_location] not in table_obj.storage.index:
            table_obj.storage.index[relation[r][attr_location]] = []
...
# match parameter signals if ON clause attribute names match.  If not, cannot remove a common attribute, so need to concatenate tuples differently
def join(relation1, relation2, attr1, attr2, typ, nested, natural_list = None, match=True):
            ...
            for r1 in relation1:
                ...
                if not found:
                    ...
                    if match:
                        ...
                        for i in range(len(relation2[0])):
                            left_helper.append(None)
            ...
            while inner_index < len(relation2) or outer_index < len(relation1):  # while still samples in both relations
                ...
                if match:
                    ...
                    if len(outer_group) > 0 and len(inner_group) > 0:  # if both are non-empty
                        for r1 in outer_group:
                            for r2 in inner_group:
                                return_relation.append(r1 + r2)
                    else:  # if one is empty, append contents of the other and pad with nulls
                        for r1 in outer_group:
                            help_list = [None for r in range(len(relation2[0]))]  # -1 to account for shared attribute
                            helper = r1 + help_list
                            return_relation.append(helper)

                        for r2 in inner_group:
                            # r2 --> pad front with nulls for the number of attributes in r1 (place r2[attr2] value in attr1 index)
                            right_helper = [None for r in range(len(relation1[0]))]
                            return_relation.append(right_helper + r2)
...
def initializer():
    ...
    #   Smaller test tables
    # # tables
    # create_table("test1_rel", [("name", "string"), ("age", "int"), ("year", "int")])
    # create_table("test2_rel", [("name", "string"), ("income", "int"), ("home_town", "string")])
    ...
    #
    # # table 2
    # inp2 = [["andrew", 400, "new york"], ["bob", 350, "dc"], ["joe", 200, "andrew"]]
